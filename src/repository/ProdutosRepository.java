...
public class ProdutosRepository {
	...
	public ProdutosRepository() {
		this.database = new File("database-pro.txt");
		...
		try (Scanner sc = new Scanner(database)) {
			while (sc.hasNextLine()) {
				...
				if (data.length >= 5) {
					...
					fly.setProduto(data[1]);
					fly.setPreco(Double.parseDouble(data[2]));
					fly.setQuantidade(Integer.parseInt(data[3]));
					fly.setCodigo(Integer.parseInt(data[4]));
					products.add(fly);
				}
			}
	...
	private void saveProducts() {
		...
		try (PrintWriter writer = new PrintWriter(new FileOutputStream(database, false))) {
			for (Fly fly : products) {
				...
				String data = fly.getId() + ";" + fly.getProduto() + ";" + fly.getPreco() + ";" + fly.getQuantidade()
						+ ";" + fly.getCodigo();
